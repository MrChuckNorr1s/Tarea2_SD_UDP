...
def enviar_tiempo_real_entrega(tiempo_real_entrega, tiempo_estimado_entrega, pedido_data):
    doc = {
        'tiempo_real_entrega': tiempo_real_entrega,
    ...
    while contador_pedidos < max_mensajes and (time.time() - start_time) < tiempo_maximo:
        ...
        if historial_estados:
            for i in range(1, len(historial_estados)):
                ...
                latencia_por_estado[f"{estado_anterior}_a_{estado_actual}"] = (timestamp_actual - timestamp_anterior) * 1000
        ...
        if pedido_data['estado'] == 'Finalizado':
            ...
            for estado, fecha in historial_estados:
                if estado == "Procesando":
                    start_timestamp = datetime.strptime(fecha, '%Y-%m-%d %H:%M:%S').timestamp()
                elif estado == "Finalizado":
                    final_timestamp = datetime.strptime(fecha, '%Y-%m-%d %H:%M:%S').timestamp()

            if start_timestamp and final_timestamp:
                tiempo_real_entrega = (final_timestamp - start_timestamp) / 60  # en minutos
